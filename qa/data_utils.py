...
def get_save_dir(args):
    if args.method == "baseline":
        # just save important info:
        save_dir = os.path.join(args.output_dir, args.train_file,
            args.model_name_or_path+f"_{args.train_batch_size}_epoch_{args.num_train_epochs}_decay_{args.linear_decay}_lr_{args.learning_rate}_seed_{args.seed}")

    elif args.method == "baseline_CR":
        save_dir = os.path.join(args.output_dir, args.train_file,
            args.model_name_or_path+\
            f"_{args.method}_alpha_{args.alpha_t}_{args.train_batch_size}_decay_{args.linear_decay}_lr{args.learning_rate}_warmup_{args.warmup_steps}_{args.warmup_epoch}_seed_{args.seed}")
    elif args.method == "baseline_CR_BC":
        save_dir = os.path.join(args.output_dir, args.train_file,
            args.model_name_or_path+\
            f"_{args.method}_alpha_{args.alpha_t}_{args.train_batch_size}_decay_{args.linear_decay}_lr{args.learning_rate}_warmup_{args.warmup_steps}_{args.warmup_epoch}_seed_{args.seed}_p_{args.bg_class_prior}")
        ...
        save_dir = os.path.join(args.output_dir, args.train_file,
            args.model_name_or_path+\
            f"_{args.method}_alpha_{args.alpha_t}_{args.train_batch_size}_decay_{args.linear_decay}_lr{args.learning_rate}_warmup_{args.warmup_steps}_{args.warmup_epoch}_seed_{args.seed}_p_{args.bg_class_prior}_d_p_{args.classifier_dropout}")        

    if args.resume_from_checkpoint_name != "":
        save_dir += "_"+args.resume_from_checkpoint_name
    if args.org_teacher:
        save_dir += "_"+f"teacher_{args.teacher_name}_temp_{args.teacher_temperature}_{args.teacher_method}"
...
class InputFeatures(object):
    def __init__(self, example_id, choices_features, label, is_synthetic=None):
        self.example_id = example_id
...
class CommonsenseqaBCProcessor(DataProcessor):
    """Processor for the Commonsenseqa data set."""
    def get_train_examples(self, data_dir, file="train.csv"):
        """See base class."""
        logger.info("LOOKING AT {} train".format(data_dir))
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, file)), "train")
    ...
    def _create_examples(self, lines, type):
        ...
        examples = [
            InputExample(
                ...
                # common beginning of each
                # choice is stored in "sent2".
                contexts=[line[1], line[1], line[1], line[1], line[1], line[1]],
                endings=[line[3], line[4], line[5], line[6], line[7], ""],
                concept=[line[2], line[2], line[2], line[2], line[2], line[2]],
                is_synthetic=[line[0] == "n/a"] * 6,
                label="0")
            for line in lines[1:]  # we skip the line with the column names
        ]
...
class WinograndeBCProcessor(DataProcessor):
    """Processor for the Commonsenseqa data set."""
    def get_train_examples(self, data_dir, file="train.csv"):
        """See base class."""
        logger.info("LOOKING AT {} train".format(data_dir))
    ...
    def _create_examples(self, lines, type):
        ...
        examples = [
            InputExample(
                ...
                endings=[line[3], line[4], ""],
                concept=[line[2], line[2], line[2]],
                is_synthetic=[line[0] == "n/a"] * 3,
                label="0")
            for line in lines[1:]  # we skip the line with the column names
        ]
...
def convert_examples_to_features(examples,
    ...
    for (ex_index, example) in tqdm.tqdm(enumerate(examples),
        ...
        for ending_idx, (context, ending) in enumerate(
            ...
            if is_winogrande:
                conj = "_"
                idx = context.index(conj)

                new_context = context[:idx]
                option_str = "_ " + context[idx + len(conj):].strip()

                new_ending = option_str.replace("_", ending)

                context = new_context
                ending = new_ending
                tokens_b = tokenizer.tokenize(new_ending)
            elif pad_qa:
                context = 'Q: ' + context
                ending = 'A: ' + ending
...
processors = {
    ...
    "winogrande_bc": WinograndeBCProcessor,
    "commonsenseqa": CommonsenseqaProcessor,
    "commonsenseqa_bc":CommonsenseqaBCProcessor,
